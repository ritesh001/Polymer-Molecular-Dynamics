  ...
  def read_files(dir: str, block_list: List[int]):
      ...
      for root, dirs, files in os.walk(dir):
          for file in files:
              ...
              if file.endswith(".txt") and len(parts) == 3:
                  ...
                  df = pd.read_csv(
                      os.path.join(dir, file), header=1,
                      usecols=[0, 1], delim_whitespace=True, names=['time', 'msd'])
      ...
      for block in block_list:
          ...
          for b in range(block):
              ...
              final_index = df_block.index[df_block['time'] == final_time].to_list()[
                  0]
  ...
  def main_calculation(block_dict, t1, t2):
      ...
      for index, num_blocks in enumerate(sorted(block_dict)):
          ...
          for n in range(num_blocks):
              x = np.log10(block_dict[num_blocks]
                          ['time'].loc[cut_point:cut_point2])
              y = np.log10(block_dict[num_blocks]
                          [f'block{n}'].loc[cut_point:cut_point2])
              x_sklearn = block_dict[num_blocks][[
                  'time']].loc[cut_point:cut_point2]
              y_sklearn = block_dict[num_blocks][f'block{n}'].loc[cut_point:cut_point2]
      ...
      return dif_mean[best_index], dif_std[best_index], msd_slope_log_mean[best_index], block_list[best_index]


  def main():
      filename = "results.csv"
      time_array = [5000000, 10000000, 20000000, 50000000, 100000000, 200000000]
      ...
      block_dict = read_files(output_dir, block_list)
      ...
      print('************** Best Result: {}-{}ns **************'.format(curr_best_t1, curr_best_t2))
      folder_name = os.path.basename(os.getcwd())
      print(folder_name, curr_best_dif_mean, curr_best_dif_std,
          curr_best_slope, curr_best_block)
      with open(filename, 'a') as csvfile:
          csvwriter = csv.writer(csvfile)
          csvwriter.writerow([folder_name, curr_best_dif_mean, curr_best_dif_std,
                              curr_best_slope, curr_best_block])
