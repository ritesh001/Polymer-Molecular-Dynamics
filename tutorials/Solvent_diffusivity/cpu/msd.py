...
def MSD(r, ir, box_bounds, id2type=[]):
	...
	cation_com = np.zeros([frames,3], float)
	anion_com = np.zeros([frames,3], float)
	cond = np.zeros(frames, float)
	...
	cond = cond/(sum(id2type == 6)+sum(id2type == 5))
...
def cond_block_avg(intrvl, nBlock, blockSize, timescale, gastype):
	if not (nBlock-1)*intrvl + blockSize == len(r):
		print("WARNING: missing some timesteps in the dump file.", file=sys.stderr)

	msd = {}
	msd_avg = {}
	cond = {}
	cond_avg = np.zeros(blockSize, float)
	...
	for n in range(nBlock):
		for type_id in sorted(msd[n]):
			if n ==0:
				msd_avg[type_id] = np.zeros(blockSize, float)
	...
	file = 'msdavged%d_tot%d_xyz.txt'% (nBlock, totFrame)
	OUT = open(file, 'w')
	OUT.write("time")
	for n in range (nBlock):
		OUT.write("	block%i" % n)
	OUT.write("	gas_avg gas_stdev\n")

	for t in range(blockSize):
		OUT.write("%8.5f" % (float(timestep[t+low]-timestep[low])*timescale))
		msd_std = []
		for n in range(nBlock):
			msd_std.append(msd[n][gastype][t])
			OUT.write("   %8.5f" % msd[n][gastype][t])
		OUT.write("	%8.5f %8.5f\n" % (msd_avg[gastype][t], np.std(msd_std)))
	OUT.close()
...
init_skip = 0
r, ir, timestep, boxbds, id2type, id2mol, mol2ids = read_lammpstrj('production_long.lammpstrj', skip_beginning=init_skip)

gas_type = 1
time_step = 1

#50000000fs; 10000frames
nframes = 20000
nblock_list = [1, 2, 4, 5, 10, 20]

for nblock in nblock_list:
	block_length = int(nframes/nblock)
	cond_block_avg(block_length, nblock, block_length+1, time_step, gas_type)
